...
def get_pblh(heights, temperatures, pressures, w_speeds, w_directs):
    ...
    try:
        ...
        while True:
            ...
            Ris.append(round(Ri/Rc, 3))
            plt_h.append(h)

            # 首次Ri／Rc＞１的高度作为边界层高度
            print("----------------------------")
            print(f"G*(h-h0)/potem: {round(G * (h - h0) / potem, 3)}, "
                  f"(potem-potem0)/(u**2+v**2): {round((potem - potem0) / (u**2 + v**2), 3)}")
            print(f"u**2+v**2:{round(u**2+v**2,  3)}, u:{round(u, 3)}, v:{round(v, 3)}, wspeed:{w_speed}, wdirect:{w_direct}")
            print(f"h: {h}, potem:{round(potem, 3)}, potem0:{round(potem0, 3)}, "
                  f"delta_potem:{round(potem - potem0, 3)}, Ri/Rc: {round(Ri/Rc, 3)}")
            if h > 3000:
                return Ris, plt_h
                # return None
            # if Ri / Rc > 1:
